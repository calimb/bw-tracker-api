...
def create_session(
    ...
    """
    Génère un programme et crée une séance.
    Reçoit : user_id, session_type, liste de skills choisis.
    Retourne : la séance créée avec les exercices générés.
    """
    # Vérification user
    ...
    # Génération du programme (réutilise votre logique de core.py)
    workout = generate_program(body.session_type, body.selected_skills, db)
    ...
    for item in workout:
        ...
        exercise = Exercise(
            ...
            sets=item["sets"],
            work_per_set=item["work_per_set"]
        )
        db.add(exercise)
    ...
    # Construction manuelle pour inclure skill_name
    return {
        "id": session.id,
        "date": session.date,
